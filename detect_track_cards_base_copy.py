...
def split_handler(x,y):
    print('before true')
    if True:
        print('entertrue')
        if x[1]>0 and x[1] < 240:
            playerHand1.append(y)
            print('hand1')
        elif x[1]>240 and x[1] < 480:
            playerHand2.append(y)
            print('hand2')
        elif x[1]>480 and x[1] < 720:
            playerHand3.append(y)
            print('hand3')
        elif x[1]>720 and x[1] < 960:
            playerHand4.append(y)
            print('hand4')
    elif True:
        if x[1]>0 and x[1] < 320:
            playerHand1.append(y)
        elif x[1]>320 and x[1] < 640:
            playerHand2.append(y)
        elif x[1]>640 and x[1] < 960:
            playerHand3.append(y)
    elif True:
        if x[1]>0 and x[1] < 480:
            playerHand1.append(y)
        elif x[1]>480 and x[1] < 960:
            playerHand2.append(y)
    elif True:
        playerHand1.append(y)

def card_to_number(cards,hand):
    for x in cards:
        if x[0].isdigit():
            if x[0] == '1':
                eval(hand.append(10))
            else:
                eval(hand.append(int(x[0])))  # this determines the value of the number
            ...
            if x[0] == 'A':
                ...
                eval(hand.append(10))
    eval(hand = sum(hand))
    if eval(hand > 21) and p_soft:
        eval(hand = hand - 10)
    else:
        eval(hand = hand)

def split_cards(xycord,labels,im0):
    for i,y in zip(xycord,range(len(xycord))):
        if i[0]<640:
            if(not dealerHand.count(labels[y])):
                print('enter dealer card')
                dealerHand.append(labels[y])
            else:
                print(xycord)
                if (not playerHand.count(labels[y])):
                    playerHand.append(labels[y])
                    print('('+str(playerHand)+')')
                    split_handler(i,labels[y])
                    print('('+str(playerHand)+')')
...
#def display_UI():
#    if (split+1 % 2) == 0:
#        cv2.rectangle(im0, (1000, 840), (1280, 960), [85, 45, 255], 30)
#        cv2.putText(im0, 'Split', (1025, 860), 0, 1, [225, 255, 255], thickness=2,lineType=cv2.LINE_AA)
...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            # money in the pot
            cv2.line(im0, (0, 920), (250, 920), [233, 94, 22], 30)
            cv2.putText(im0, 'wallet: $' + str(wallet), (25, 930), 0, 1, [225, 255, 255], thickness=2,lineType=cv2.LINE_AA)
...
#######FOR GOD SAKE EXTRACT THIS INTO A DIFFERENT FUNCTION
#THE NEXT 2 PARAGRAPHS DO THE SAMETHING AND CAN BE PARAMATERISED
                d_soft = False
                p_soft = False
                #card_to_number(playerHand1,"playerHand1")
                ...
                if not game_over:
                    ...
                    if not 10 < 1 and dealer_total > 17:
                ...
                cv2.line(im0, (0, 25), (220, 25), [85, 45, 255], 30)
                cv2.putText(im0, 'Dealer has ' + str(dealer_cal), (5, 35), 0, 1, [225, 255, 255], thickness=2,lineType=cv2.LINE_AA)

                cv2.line(im0, (1060, 25), (1280, 25), [233, 94, 22], 30)
                cv2.putText(im0, 'Player has ' + str(blackjack_cal), (1060, 35), 0, 1, [225, 255, 255], thickness=2,lineType=cv2.LINE_AA)

                #player stratigy display
                cv2.line(im0, (1000, 920), (1280, 920), [233, 94, 22], 30)
                cv2.putText(im0, 'Player ' + strat, (1000, 930), 0, 1, [225, 255, 255], thickness=2, lineType=cv2.LINE_AA)
